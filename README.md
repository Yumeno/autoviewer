...
* **v1.12**  
  * 【機能追加】Windows のタッチパネル / Surface 等で **2 本指ピンチイン / ピンチアウト** によるズーム操作を追加。スライダー / Ctrl + ホイールと同じ「指の中点を軸にしたズーム」として動作します。
  * Windows ネイティブの WM_GESTURE (GID_ZOOM) を pywin32 + ctypes で hook する実装。Windows 環境では `pywin32` が依存に追加されます (起動スクリプトの初回 install で自動導入)。macOS / Linux ではピンチは無効 (silent) で、`pywin32` もインストールされません (環境マーカで除外)。
  * Windows でスライダー / Ctrl + ホイールでのズーム操作は変わらず利用できます。
* **v1.11**  
