...
* **画面の中央をタップ（クリック）**: 画面下部に**操作パネル**が表示されます。ここからシークバー操作、停止 / 再開、スライドショー間隔の調整、最大化切替、対象フォルダ変更、サムネイル帯表示、情報欄表示ができます。もう一度中央をタップするか、パネルの「閉じる」で消えます。
...
* **操作パネルの「情報欄表示」**: 画像の右端に重なるオーバーレイ型の情報欄を表示できます。A1111 WebUI の PNG info や EXIF など、読み取れる範囲の情報を画像切替に合わせて更新します。
