...
* **v1.13**  
  ...
  * 操作パネル: **compact (常時) + 詳細展開** の 2 段階構造。compact は seekbar + 再生制御のみで鑑賞中も邪魔にならず、「詳細 ▼」ボタンで VIEW/FILE/ZOOM/INTERVAL/EXIT のフル機能が展開されます。
  * 監視 ON/OFF、サムネ、情報欄の各トグルは状態ドット (●=ON / ○=OFF) で表示するように。
  * サムネ帯の現在画像強調をアクセント色に変更。
  * ウィンドウタイトルを `autoviewer` に統一 (リポジトリ名と同じ)。
  * 機能・キーバインド・操作仕様に変更はなし (見た目と配置のみの刷新)。
* **v1.12**  
  * 【機能追加】画像上のダブルタップ / ダブルクリックで「タップ位置を中心としたズーム」をトグルできるように。1.0x → 2.0x → 4.0x → 1.0x の順にサイクル。鑑賞中に操作パネルを開かずにアンカー指定ズームができます。
  * 単一タップ (左/右/中央タップ) は引き続き動作しますが、ダブルタップ判定のため 300ms 程度の遅延が入ります (スワイプは即時)。
  * 【非対応】Windows ピンチ操作は Python 3.13+ で Tk の WndProc サブクラス化が Fatal error になる制約のため断念しました。タッチ環境ではダブルタップを使ってください。
* **v1.11**  
  * 【機能追加】表示中画像を拡大して細部を確認できるズーム機能を追加。フィット表示 (1.0x) を基準に最大 5.0x までの相対倍率。
  * 【機能追加】再生中の操作パネル末尾に「ズーム」スライダー、現在倍率表示、「リセット」ボタンを追加。
  * 【機能追加】Ctrl + マウスホイールで、カーソル位置を軸にズームイン/アウト。
  * 【機能追加】ズーム中 (>1.0x) は画像領域のドラッグで注視位置を変更できます。タップ/スワイプは pan に専念するため一時的に無効化されます。
  * 【挙動】画像を切り替えると自動的にズーム 1.0x (フィット表示) と中央位置へ戻ります。
  * 【非対応 (将来対応予定)】タッチパネルのピンチ操作はこの版では未実装。次バージョンで Windows のみ ctypes で対応予定。
* **v1.10**  
  * 【機能追加】再生中の操作パネルに「ファイラで表示」「コピー」ボタンを追加。既存の右クリックメニューと同等の操作が、タッチパネル / タブレット / リモートデスクトップ等の右クリック困難な環境でも実行できます。クリップボードコピーは引き続き Windows のみ動作します。
* **v1.9**  
...
* **v1.8**  
  ...
  * 【機能追加】再生中の操作パネルに「監視ON/OFF」トグルと「即時チェック」ボタンを追加。即時チェックは 100ms 間隔で 2 回スキャンし、書き込みが安定したファイルだけを取り込みます。  
  * 【改善】ファイル上書き・削除を検知して、表示中画像のメタ情報や画像リストを正しく更新するようにしました (旧版では検知できず潜在的に壊れていた)。  
  * 【改善】`watchdog` 依存を撤去し、`requirements.txt` を `Pillow` 1 本に整理。
* **v1.7**  
